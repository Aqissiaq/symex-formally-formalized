...
Module BasicMaps.
  ...
  Fixpoint Aapply (s:string -> Aexpr) (e:Aexpr) : Aexpr :=
    ...
    | AIte b a1 a2 => AIte (Bapply s b) (Aapply s a1) (Aapply s a2)
    end
  with Bapply  (s:string -> Aexpr) (e:Bexpr) : Bexpr :=
         match e with
         | BTrue => BTrue
         | BFalse => BFalse
         | BNot b => BNot (Bapply s b)
         | BAnd b1 b2 => BAnd (Bapply s b1) (Bapply s b2)
         | BOr b1 b2 => BOr (Bapply s b1) (Bapply s b2)
         | BLeq a1 a2 => BLeq (Aapply s a1) (Aapply s a2)
         end.

  Lemma apply_id:
      (forall e, Aapply id_sub e = e) /\ (forall b, Bapply id_sub b = b).
  Proof.
    apply aexpr_bexpr_mutind with (P := fun e => Aapply id_sub e = e) (P0 := fun b => Bapply id_sub b = b);
      intros; simpl;
      try rewrite H;
      try rewrite H0;
      try rewrite H1;
  ...
  Corollary Aapply_id: forall e,
      Aapply id_sub e = e.
  Proof. apply apply_id. Qed.

  Corollary Bapply_id : forall e,
      Bapply id_sub e = e.
  Proof. apply apply_id. Qed.
  ...
  Fixpoint Aeval (V:Valuation) (e:Aexpr) : nat :=
    ...
    | AIte b a1 a2 => if (Beval V b) then (Aeval V a1) else (Aeval V a2)
    end with Beval (V:Valuation) (e:Bexpr) : bool :=
               match e with
               | BTrue => true
               | BFalse => false
               | BNot b => negb (Beval V b)
               | BAnd b1 b2 => (Beval V b1) && (Beval V b2)
               | BOr b1 b2 => (Beval V b1) || (Beval V b2)
  ...
  Lemma comp_sub_both: forall V s,
      (forall e, Aeval (Comp V s) e = Aeval V (Aapply s e))
      /\ forall b,Beval (Comp V s) b = Beval V (Bapply s b).
  Proof.
    intros V s. apply aexpr_bexpr_mutind with
      (P := fun e => Aeval (Comp V s) e = Aeval V (Aapply s e))
      (P0 := fun b => Beval (Comp V s) b = Beval V (Bapply s b));
      intros; simpl;
      try rewrite H;
      try rewrite H0;
      try rewrite H1;
  ...
  (** Lemma 2.1 *)
  Corollary comp_sub : forall V s e,
      Aeval (Comp V s) e = Aeval V (Aapply s e).
  Proof. apply comp_sub_both. Qed.

  Corollary comp_subB : forall V s e,
